use nom::{IResult, branch::alt, character::complete::{none_of, one_of}, combinator::recognize, error::{ParseError, context}, multi::{count, many0}, sequence::{delimited, pair, tuple}};
...
impl<'a> Parse<'a> for RuneLit<'a> {
    fn parse(s: Span<'a>) -> IResult<Span<'a>, Self, Self::Error> {
        let (s, rune) = context("RuneLit", ws(recognize(delimited(
            tag("'"),
            alt((
...
impl<'a> Parse<'a> for StringLit<'a> {
    fn parse(s: Span<'a>) -> IResult<Span<'a>, Self, Self::Error> {
        let (s, strlit) = context("StringLit", ws(recognize(delimited(
            tag("\""),
            many0(alt((
                recognize(pair(tag("\\U"), count(one_of("0123456789abcdefABCDEF"), 8))),
                recognize(pair(tag("\\u"), count(one_of("0123456789abcdefABCDEF"), 4))),
...
impl<'a> Parse<'a> for RawLit<'a> {
    fn parse(s: Span<'a>) -> IResult<Span<'a>, Self, Self::Error> {
        let (s, rawlit) = context("RawLit", ws(recognize(delimited(
            tag("`"),
            many0(none_of("`")),
            tag("`"),
        ))))(s)?;
